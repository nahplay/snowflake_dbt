...
{#/* Used in conjunction with generate_schema_name, this macro returns a schema name
        based on the current working environment
       ARGS:
  ...
  {%- else -%}
    {%- if custom_schema_name is none -%}
      {{ _normalize_schema(branch_name) }}_{{ default_schema }}
    {%- else -%}
      {{ _normalize_schema(branch_name) }}_{{ custom_schema_name | trim }}
    {%- endif -%}
  {%- endif -%}
{%- endmacro %}
...
{% macro generate_schema_name(custom_schema_name, node) -%}
    {{ env_generate_schema_name(custom_schema_name, branch_name = env_var('GIT_BRANCH', ''), default_schema = target.schema) }}
{%- endmacro -%}